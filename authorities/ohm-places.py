...
def run_integrated_boundary_pass(pbf_file):
    """Complete OHM boundary relation geometry in-place in the places index."""
    from processing.osm_boundary_geometry import run_boundary_pass

    print()
    print("-" * 80)
    print("OHM FULL-GEOMETRY COMPLETION (integrated boundary pass)")
    print("-" * 80)
    ...
    run_boundary_pass(pbf_file, namespace='ohm', places_index='places')
    es.indices.refresh(index='places')
