...
# ---------------- NUMERIC FEATURE ID ENCODING ----------------
# Tippecanoe and MapLibre GL JS require integer feature IDs.
# We encode namespace + OSM relation ID into a single integer that
# fits within JavaScript's Number.MAX_SAFE_INTEGER (2^53 - 1).
#
# Layout (53 bits total):
#   bits 52-48 (5 bits): namespace code  (0=osm, 1=ohm, 2=m49, 3-31 reserved)
#   bits 47-0  (48 bits): relation ID    (max ~281 trillion)
#
# This is fully decodable on both Python and JS sides.

NAMESPACE_CODES = {'osm': 0, 'ohm': 1, 'm49': 2}
NAMESPACE_NAMES = {v: k for k, v in NAMESPACE_CODES.items()}
_NS_SHIFT = 48
_ID_MASK = (1 << 48) - 1  # 0xFFFF_FFFF_FFFF


def encode_feature_id(namespace, relation_id):
    """Encode namespace + relation ID into a single integer for vector tile feature IDs.

    >>> encode_feature_id('osm', 12345)
    12345
    >>> encode_feature_id('ohm', 12345)
    281474976722809
    >>> encode_feature_id('m49', 1)
    562949953421313
    >>> decode_feature_id(encode_feature_id('ohm', 99999))
    ('ohm', 99999)
    >>> decode_feature_id(encode_feature_id('m49', 42))
    ('m49', 42)
    """
    ns_code = NAMESPACE_CODES[namespace]
    return (ns_code << _NS_SHIFT) | (relation_id & _ID_MASK)


def decode_feature_id(numeric_id):
    """Decode a numeric feature ID back to (namespace, relation_id).

    >>> decode_feature_id(12345)
    ('osm', 12345)
    >>> decode_feature_id(281474976722809)
    ('ohm', 12345)
    >>> decode_feature_id(562949953421313)
    ('m49', 1)
    """
    ns_code = (numeric_id >> _NS_SHIFT) & 0x1F  # 5 bits
    relation_id = numeric_id & _ID_MASK
