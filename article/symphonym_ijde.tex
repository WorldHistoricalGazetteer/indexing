...
\paragraph{Scope and Application.} Symphonym addresses the \emph{name-matching} component of toponym resolution. The model operates purely on phonetic similarity between strings, with no access to geographic coordinates or spatial context. Within a reconciliation pipeline, phonetic similarity serves as one input to a larger process: candidate toponyms are retrieved via the phonetic-aware index, then filtered by geographic proximity and other contextual criteria. The contribution here is the phonetic similarity component alone.

Within a complete reconciliation workflow, the pipeline proceeds in stages: the 67 million toponyms from consolidated gazetteer sources are embedded by the Student encoder and pre-indexed via HNSW; at query time, an incoming toponym is encoded to the same 128-dimensional space and matched against the index by approximate nearest-neighbour search; candidate sets retrieved by phonetic similarity are then filtered by geographic proximity, entity type, and temporal constraints to produce final matches.

In practice, the system will enable two important capabilities for users of a major consolidated historical gazetteer. First, researchers will be able to search for a toponym by entering an approximate phonetic rendering in their own language and script---a Greek scholar typing ``Ιεροσόλυμα'' will retrieve results for Jerusalem across Arabic, Hebrew, Latin, and Cyrillic attestations without needing to know the name in each script. Second, cultural heritage professionals using the platform's reconciliation API will be better able to identify places mentioned in archival descriptions or transcripts of historical documents, where place names may appear in unfamiliar scripts or non-standard orthographies.
...
\paragraph{Scalability.} Symphonym embeddings integrate with Elasticsearch's HNSW approximate nearest-neighbour indexing~\citep{malkov2020hnsw}, enabling sub-second retrieval over 67M toponyms. Query latency averages 15--50ms. The Student model's inference cost is minimal: encoding a toponym requires a single forward pass through an 8.3M-parameter network ($<$1ms on CPU). This operational efficiency is a key contribution: the system runs at production scale with acceptable latency, requiring only raw character input with no runtime phonetic conversion or language identification.
...
\paragraph{Same-script Performance.} Same-script cross-language pairs achieve 86\% pass rate, reflecting correct phonetic discrimination: exonyms with genuinely different pronunciations receive appropriately low scores. This behaviour is operationally desirable---phonetically divergent same-script variants are linked through place-level attestations rather than phonetic similarity, while traditional string methods (Levenshtein, Jaro-Winkler) complement Symphonym for same-script refinement.

\paragraph{Operational Implications.} Symphonym embeddings integrate directly with Elasticsearch's HNSW approximate nearest-neighbour indexing, enabling sub-second retrieval over the full 67M-toponym corpus. Unlike rule-based phonetic algorithms (Soundex, Metaphone), which require exact code matches and operate within single scripts, embedding similarity enables fuzzy matching with graceful degradation across all scripts simultaneously. The Student encoder requires only raw character input---no language identification, no G2P conversion, no phonetic databases---which simplifies deployment and eliminates failure modes associated with unknown languages or unsupported scripts. In a production reconciliation pipeline, Symphonym serves as one component: cross-script candidate retrieval benefits from phonetic embeddings; same-script refinement may use edit distance; final disambiguation incorporates geographic proximity and temporal constraints. The embedding similarity score provides a phonetic prior that downstream components weigh against other evidence.
...
By learning phonetic correspondences that are difficult to enumerate manually, Symphonym's fixed-length embeddings address a practical and growing challenge in geospatial data integration: linking name references across languages, scripts, and historical periods at production scale.

\subsection*{Acknowledgments}

[Anonymised for review.]

In accordance with Taylor \& Francis policy, the following uses of generative AI tools are disclosed. Claude (Anthropic), GPT-5 (OpenAI),
...
All trained models, vocabularies, extension files, and evaluation results are openly available at [Anonymised for review---Zenodo DOI will be provided]. Training code and inference utilities are also available at [Anonymised for review---HuggingFace and GitHub URLs will be provided]. Training data are derived from publicly available sources: GeoNames (\url{https://www.geonames.org/}), Wikidata (\url{https://www.wikidata.org/}), and Getty TGN (\url{https://www.getty.edu/research/tools/vocabularies/tgn/}). The MEHDIE benchmark is available via \citet{mehdie2025}.
...
\midrule
\textbf{PanPhon192} & TS7 (Yaqut-Kima Sham) & 15.2 & 21.2 & 27.3 & 19.2 \\
\textbf{PanPhon192} & TS8 (Kima-Thurayya Sham) & 28.6 & 28.6 & 42.9 & 32.3 \\
\textbf{PanPhon192} & TS9 (Tudela-Thurayya) & 77.8 & 88.9 & 88.9 & 82.0 \\
\textbf{PanPhon192} & TS10 (Yaqut-Kima Maghreb) & 12.1 & 21.2 & 21.2 & 16.6 \\
\textbf{PanPhon192} & TS11 (Damast-Tudela) & 71.9 & 81.2 & 81.2 & 75.1 \\
\textbf{PanPhon192} & \textbf{Mean} & \textbf{41.1} & \textbf{48.2} & \textbf{52.3} & \textbf{45.0} \\
\midrule
\textbf{Levenshtein} & TS7 (Yaqut-Kima Sham) & 75.8 & 93.9 & 100.0 & 83.7 \\
\textbf{Levenshtein} & TS8 (Kima-Thurayya Sham) & 90.5 & 100.0 & 100.0 & 94.4 \\
\textbf{Levenshtein} & TS9 (Tudela-Thurayya) & 77.8 & 100.0 & 100.0 & 87.5 \\
\textbf{Levenshtein} & TS10 (Yaqut-Kima Maghreb) & 66.7 & 97.0 & 97.0 & 79.6 \\
\textbf{Levenshtein} & TS11 (Damast-Tudela) & 96.9 & 96.9 & 100.0 & 97.3 \\
\textbf{Levenshtein} & \textbf{Mean} & \textbf{81.5} & \textbf{97.5} & \textbf{99.4} & \textbf{88.5} \\
\midrule
\textbf{Jaro-Winkler} & TS7 (Yaqut-Kima Sham) & 75.8 & 100.0 & 100.0 & 86.4 \\
\textbf{Jaro-Winkler} & TS8 (Kima-Thurayya Sham) & 90.5 & 90.5 & 95.2 & 91.4 \\
\textbf{Jaro-Winkler} & TS9 (Tudela-Thurayya) & 77.8 & 100.0 & 100.0 & 86.6 \\
\textbf{Jaro-Winkler} & TS10 (Yaqut-Kima Maghreb) & 54.5 & 93.9 & 97.0 & 71.9 \\
\textbf{Jaro-Winkler} & TS11 (Damast-Tudela) & 93.8 & 96.9 & 96.9 & 95.4 \\
\textbf{Jaro-Winkler} & \textbf{Mean} & \textbf{78.5} & \textbf{96.2} & \textbf{97.8} & \textbf{86.3} \\
\midrule
\textbf{Symphonym} & TS7 (Yaqut-Kima Sham) & 69.7 & 97.0 & 100.0 & 82.9 \\
\textbf{Symphonym} & TS8 (Kima-Thurayya Sham) & 95.2 & 100.0 & 100.0 & 96.8 \\
\textbf{Symphonym} & TS9 (Tudela-Thurayya) & 94.4 & 100.0 & 100.0 & 97.2 \\
\textbf{Symphonym} & TS10 (Yaqut-Kima Maghreb) & 72.7 & 87.9 & 87.9 & 80.8 \\
\textbf{Symphonym} & TS11 (Damast-Tudela) & 93.8 & 100.0 & 100.0 & 96.4 \\
\textbf{Symphonym} & \textbf{Mean} & \textbf{85.2} & \textbf{97.0} & \textbf{97.6} & \textbf{90.8} \\
\bottomrule
\end{tabular}
