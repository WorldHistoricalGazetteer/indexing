...
\paragraph{Phase 3 (Hard Negative Fine-tuning).} Fine-tuning on 3.3M hard negative triplets for 17 epochs further improved discrimination. Validation loss decreased from 0.0149 to 0.0051---a 66\% reduction---demonstrating that the model learned to distinguish phonetically similar but geographically distinct names. Training continued with a reduced learning rate ($10^{-4}$ with cosine decay) to preserve cross-script alignment learned in Phase 2.
...
\midrule
Cross-script equivalents & 29/29 (100\%) & Latin $\leftrightarrow$ Cyrillic, Greek, Arabic, CJK, etc. \\
Same-script cross-language & 12/14 (86\%) & London/Londres, Vienna/Wien \\
Unrelated pairs & 15/15 (100\%) & Correctly separated ($<0.50$) \\
Diacritic variants & 10/10 (100\%) & Zurich/Zürich, Gdansk/Gdańsk \\
\midrule
\textbf{Total} & \textbf{66/68 (97.1\%)} & \\
\bottomrule
\end{tabular}
\caption{Embedding quality diagnostics after Phase 3 training. Cross-script matching is the primary design goal; the model achieves perfect accuracy on this category while maintaining strong performance on related tasks.}
\label{tab:embedding-quality}
\end{table}

\paragraph{Cross-script Equivalents.} The model achieves perfect performance on cross-script matching, the primary design goal. Representative pairs and their cosine similarities:

\begin{itemize}
    \item London / Лондон (Cyrillic): 0.995
    \item Beijing / \textcn{北京} (Chinese): 0.966
    \item Seoul / \textko{서울} (Korean): 0.986
    \item Jerusalem / \texthebrew{ירושלים} (Hebrew): 0.980
    \item Tbilisi / \textgeorgian{თბილისი} (Georgian): 0.976
    \item Delhi / \textdevanagari{दिल्ली} (Devanagari): 0.950
\end{itemize}

\paragraph{Same-script Cross-language.} Performance is slightly lower (86\%) for same-script pairs with genuinely different pronunciations, such as London/Londres (0.480) and London/Londra (0.592). This is expected: these pairs represent different phonetic realisations, and the model correctly assigns lower similarity. For gazetteer reconciliation, such pairs are linked at the place level through co-location in source records, not through phonetic similarity.

\paragraph{Historical Variants.} As noted in Section~\ref{sec:training}, the model captures synchronic phonetic similarity rather than diachronic derivation. Pairs like Istanbul/Constantinople (0.681) and Paris/Lutetia (0.398) correctly receive moderate to low similarity; historical linkage is handled through curated place attestations.

\paragraph{Script-Pair Analysis.} Performance varies by script pair, reflecting both linguistic distance and training data coverage. Latin$\leftrightarrow$Cyrillic pairs achieve near-perfect similarity (mean 0.97) due to abundant training examples and close phonetic correspondence. Latin$\leftrightarrow$CJK pairs also perform strongly (mean 0.98) because romanisation (Pinyin, Romaji) creates direct phonetic mappings. More challenging are pairs involving scripts with complex orthography-phonology relationships: Latin$\leftrightarrow$Arabic achieves 0.91 mean similarity, reflecting the omission of short vowels in Arabic script.
...
\textbf{Symphonym} & \textbf{0.892} & \textbf{0.988} & 0.988 & \textbf{0.930} \\
\bottomrule
\end{tabular}
...
Symphonym achieves 89.2\% Recall@1---the correct cross-script match is ranked first in nearly 9 out of 10 queries---compared to 81.5\% for Levenshtein and 78.5\% for Jaro-Winkler. The improvement is most pronounced on testset 10 (Yaqut $\leftrightarrow$ Kima), where Symphonym achieves 84.8\% R@1 compared to 66.7\% for Levenshtein. Table~\ref{tab:mehdie-per-testset} breaks down results by testset.
...
\midrule
TS7 (Yaqut-Kima Sham) & 0.758 & 0.758 & 0.697 & 0.837 & 0.864 & 0.812 \\
TS8 (Kima-Thurayya Sham) & 0.905 & 0.905 & \textbf{1.000} & 0.944 & 0.914 & \textbf{1.000} \\
TS9 (Tudela-Thurayya) & 0.778 & 0.778 & \textbf{0.944} & 0.875 & 0.866 & \textbf{0.972} \\
TS10 (Yaqut-Kima Maghreb) & 0.667 & 0.545 & \textbf{0.848} & 0.796 & 0.719 & \textbf{0.888} \\
TS11 (Damast-Tudela) & 0.969 & 0.938 & 0.969 & 0.973 & 0.954 & 0.977 \\
\bottomrule
\end{tabular}
...
\paragraph{Cross-script Generalisation.} The model achieves strong accuracy on cross-script equivalents in diagnostic tests (29/29 pairs) and 89.2\% Recall@1 on the MEHDIE Hebrew-Arabic benchmark. The MEHDIE evaluation is particularly notable because the benchmark sources (medieval Hebrew and Arabic geographical texts) are independent of the modern gazetteers used for training, even though Hebrew and Arabic scripts appear in the training data through GeoNames and Wikidata entries. The strong performance suggests the model has learned general cross-script phonetic mappings rather than memorising specific toponyms. This generalisation emerges from the Teacher-Student architecture: the Teacher learns language-specific phonetic mappings from IPA features, then transfers this knowledge to the Student through distillation.
...
\paragraph{Same-script Limitations.} Performance on same-script cross-language pairs (86\% pass rate) is lower than cross-script pairs (100\%). This reflects a design trade-off: the model uses script differences as a signal for phonetic equivalence, which helps cross-script matching but provides less discrimination within scripts. For same-script matching, traditional string similarity methods (Levenshtein, Jaro-Winkler) remain effective and can complement Symphonym in a hybrid pipeline.

\paragraph{Hard Negative Training.} Phase 3 training substantially improved discrimination (validation loss reduced 66\% from 0.015 to 0.005) without degrading cross-script performance. The prefix-based negative sampling strategy (Section~\ref{subsec:phase3}) proved effective: by selecting negatives that share orthographic prefixes but represent different places, the model learned to distinguish phonetically similar but geographically distinct names.
...
\begin{itemize}
    \item \textbf{97\% accuracy} on cross-script diagnostic pairs (66/68), spanning Latin, Cyrillic, Greek, Arabic, Chinese, Korean, Hebrew, Devanagari, and Georgian scripts
    \item \textbf{89.2\% Recall@1} and \textbf{MRR of 0.930} on the MEHDIE Hebrew-Arabic benchmark---comprising medieval toponyms from sources independent of training data---outperforming Levenshtein (81.5\% R@1) and Jaro-Winkler (78.5\% R@1) baselines
\end{itemize}
