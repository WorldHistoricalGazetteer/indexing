...
\paragraph{Training Status.} Phase 1 (Teacher) training commenced February 13, 2026, on NVIDIA L40S GPUs and is currently in progress (resumable checkpoints are saved per epoch). Phase 2 and Phase 3 training are scheduled to follow upon Phase 1 completion. Full Epitran extension coverage has increased the usable toponym count from 29M (v5) to approximately 29.3M (v6), adding critical representation for Greek, Hebrew, Armenian, and Gujarati toponyms. This is expected to eliminate the script-specific performance gaps observed in v5 MEHDIE testing (Section~\ref{sec:discussion}), where Greek and Hebrew pairs showed anomalously low similarity scores due to insufficient training data.
...
\midrule
LATIN & 55,617,677 & 83.1\% & 43.7\% & en, fr, nl, de, sv \\
CYRILLIC & 3,614,762 & 5.4\% & 40.8\% & ru, uk, sr \\
CJK & 2,973,525 & 4.4\% & 43.9\% & zh (Charsiu) \\
ARABIC & 2,098,089 & 3.1\% & 52.4\% & fa, ar, ur \\
HANGUL & 393,996 & 0.6\% & 58.0\% & ko (Charsiu) \\
OTHER & 342,642 & 0.5\% & 0.0\% & --- \\
KATAKANA & 340,555 & 0.5\% & 0.0\% & ja (ext. added) \\
THAI & 251,458 & 0.4\% & 83.6\% & th \\
GREEK & 217,997 & 0.3\% & 0.0\% & el (ext. added) \\
DEVANAGARI & 166,957 & 0.2\% & 51.1\% & hi, mr \\
ARMENIAN & 153,467 & 0.2\% & 0.0\% & hy (ext. added) \\
HIRAGANA & 151,980 & 0.2\% & 0.0\% & ja (ext. added) \\
...
KANNADA & 43,155 & 0.1\% & 0.0\% & kn (ext. added) \\
GUJARATI & 21,428 & 0.03\% & 0.0\% & gu (ext. added) \\
\bottomrule
\end{tabular}
\caption{Script distribution across all 66.9M unique toponyms. IPA coverage percentages reflect successful transcription within the 57.6M training namespace toponyms (gn/wd/tgn). v6 uses Epitran extensions for 97 new language-script pairs, Phonikud for Hebrew, and CharsiuG2P for Chinese topolects/Korean. Scripts showing 0.0\% coverage indicate Epitran extension files exist but IPA generation failed or was not properly tracked during pipeline execution; these toponyms contribute to Phase 2--3 character-level training.}
...
\paragraph{Enhanced Effective Training Set.} From the 65.1 million positive pairs generated via HDBSCAN clustering across 8.2 million places (forming 18.3 million phonetic clusters, with 4.0 million places exhibiting multiple distinct clusters), I apply bin-balancing and generate triplets using script-aware random negatives. Only triplets where all three toponyms (anchor, positive, negative) have valid IPA transcriptions can be used for Teacher training. From 15.9 million unique toponym IDs referenced in pairs, the filtering yields \textbf{29,927,920 Phase 1 triplets}, split deterministically into \textbf{26,927,553 training} and \textbf{3,000,367 validation} samples (90:10 ratio).
...
Epoch 1 val\_loss & 0.0109 & 0.0131 \\
Epoch 5 val\_loss & 0.0064 & 0.0080 \\
Epoch 10 val\_loss & 0.0054 & 0.0069 \\
Epoch 15 val\_loss & 0.0050 & 0.0064 \\
Epoch 20 val\_loss (v5 best) & 0.0048 & [TBD] \\
\bottomrule
\end{tabular}
\caption{Phase 1 Teacher training: v5 vs v6 comparison. v6 val\_loss is consistently $\sim$1.28$\times$ higher than v5, reflecting the harder discrimination task with native script phonetic representations rather than romanised approximations. v5 used AnyAscii romanisation for CJK; v6 processes CJK, Hebrew, and Greek natively via Charsiu, Phonikud, and extended Epitran respectively.}
\label{tab:phase1-convergence}
\end{table}

The v6 model shows stable convergence with no divergence, in contrast to an earlier v5 attempt at learning rate $10^{-3}$ which diverged after epoch 1. The consistently higher v6 loss reflects the genuinely harder task: the model must now discriminate between real phonetic representations of Chinese, Hebrew, and Greek toponyms rather than relying on lossy AnyAscii romanisations. A v6 val\_loss of 0.0064 at epoch 15 represents a stronger Teacher than v5's 0.0064 at the same epoch, because the underlying phonetic space is richer and the discrimination more demanding.
...
\paragraph{Version 6 Improvements.} The transition from v5 to v6 addresses the critical coverage gaps identified during v5 evaluation. The hybrid G2P strategy---combining 97 new Epitran extension files, Phonikud for Hebrew, and CharsiuG2P for CJK topolects---expands IPA coverage from approximately 29M to an estimated 29.3M toponyms. The elimination of AnyAscii romanisation for CJK scripts means the Teacher now learns from genuine phonetic representations rather than lossy approximations: Chinese /bei.dʒiŋ/ rather than romanised "beijing", Hebrew /jəruʃaˈlajim/ from unpointed ירושלים rather than no embedding at all. Phase 1 training on the expanded v6 dataset shows val\_loss consistently $\sim$1.28$\times$ higher than v5 at equivalent epochs, reflecting the harder but more linguistically faithful discrimination task. The additional 2.1M training triplets (26.9M vs 24.8M) arise from the expanded IPA coverage of previously excluded scripts. These improvements are expected to eliminate the script-specific performance gaps observed in v5 MEHDIE testing, where Hebrew pairs showed anomalously low similarity scores due to absent training data.
