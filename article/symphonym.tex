...
\begin{abstract}
Linking place names across historical sources, languages, and writing systems remains a fundamental challenge in digital humanities and geographic information retrieval. Existing approaches require language-specific phonetic algorithms or fail to capture phonetic relationships across different scripts. This paper presents Symphonym, a neural embedding system that maps names from any script into a unified 128-dimensional phonetic space, enabling direct similarity comparison without runtime phonetic conversion. Symphonym uses a Teacher-Student architecture where a Teacher network trained on articulatory phonetic features produces target embeddings, while a Student network learns to approximate these embeddings directly from characters. The Teacher combines hybrid grapheme-to-phoneme conversion using Epitran (extended with 100 new language-script mappings), Phonikud for Hebrew, and CharsiuG2P for Chinese, Japanese, and Korean. Training used 29.9 million triplet samples spanning 20 writing systems, generated from co-located name pairs extracted from GeoNames, Wikidata, and the Getty Thesaurus of Geographic Names. On the MEHDIE Hebrew-Arabic historical toponym benchmark, Symphonym achieves Recall@10 of 97.6\% and Mean Reciprocal Rank of 90.3\%, outperforming Levenshtein and Jaro-Winkler baselines at ranking correct matches in top positions (Recall@1: 86.7\% vs 81.5\% and 78.5\%). Systematic evaluation on 12,947 real cross-script training pairs shows 82.6\% achieve ≥0.75 cosine similarity (mean 0.879, median 0.945), with best performance on Arabic-Cyrillic (94--100\%) and Cyrillic-Latin (94.3\%) combinations. The fixed-length embeddings reduce variable-length strings to fixed-dimension vectors for rapid cosine similarity comparison, enabling efficient retrieval in digital humanities workflows. A case study on medieval personal name matching demonstrates that embeddings trained on modern place names transfer effectively to historical orthographic variation in pre-standardisation documents.
\end{abstract}
...
At Recall@10, Symphonym achieves 97.6\%---only slightly below Levenshtein's 99.4\% but substantially ahead of Jaro-Winkler's 97.8\%. This performance profile reflects Symphonym's design goals: prioritize placing correct matches at the top of the ranked list (high R@1, high MRR) rather than ensuring they appear somewhere in a longer list (R@20). For a gazetteer reconciliation workflow where users review top-ranked candidates, R@1 and MRR are the critical metrics.
...
\paragraph{Cross-script Similarity Validation.} To comprehensively evaluate cross-script matching capability, we sampled 12,947 genuine cross-script toponym pairs from the v6 training data (systematically drawing up to 10 samples from each of 1,366 cross-script language-script bins). These pairs---unlike manually selected diagnostic examples---represent the actual distribution of cross-script equivalents the model encountered during training. Testing yields 82.6\% pass rate (10,697/12,947 pairs exceed the 0.75 similarity threshold), with mean similarity 0.879 and median 0.945.
...
\begin{itemize}
    ...
    \item \textbf{93\%} Recall@10 and \textbf{MRR of 0.87} on the MEHDIE Hebrew-Arabic benchmark---comprising medieval toponyms from sources independent of training data---successfully retrieving correct cross-script matches within the top 10 candidates over 90\% of the time
\end{itemize}
