...
\begin{itemize}
    ...
    \item \textbf{Cluster 2 (8 members):} South Asian and Middle Eastern forms cluster together: Tamil \textta{மாஸ்கோ}, Arabic \textar{موسكو}, Devanagari \texthi{मॉस्को} (mr, hi), Thai \textth{มอสโก}, Bengali \textbn{মস্কো}, and Telugu \textte{మాస్కో}---capturing the /masko/ pronunciation variant common to these language families (similarity 0.85)
    \item \textbf{Noise (5 points):} Georgian \textge{მოსკოვი}, CJK \textcn{莫斯科}, and some Arabic variants are correctly isolated as phonetically distinct
\end{itemize}

A \textbf{Beijing} place record (gn:1816670) with 36 toponyms demonstrates nuanced phonetic clustering across 3 groups:

\begin{itemize}
    \item \textbf{``Beijing'' cluster (10 members):} Chinese \textcn{北京}, Korean \textko{베이징}, Thai \textth{ปักกิ่ง}, Tamil \textth{பெய்ஜிங்}, Vietnamese ``Bắc Kinh'', and Latin Beijing forms---all sharing the /bei-dʒiŋ/ phonetic pattern (similarity 0.83)
    ...
    \item \textbf{``Pechino/Pekin'' cluster (9 members):} Georgian \textge{პეკინი}, Italian ``Pechino'', and additional Latin variants---capturing the /pek-/ phonetic core across scripts (similarity 0.72)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{``Parigi'' cluster (5 members):} Italian ``Parigi'', Finnish ``Pariisi'', Dutch ``Parijs'', Malayalam \textth{പാരിസ്}, Georgian \textge{პარიზი}---the /pariʒi/ variant
\end{itemize}
...
\paragraph{Bin Balancing.} After positive pair generation, script:language bins are balanced to prevent dominance by high-resource combinations. Of the 595 bins, 264 exceed the 50,000 sample cap and are downsampled; 329 smaller bins are oversampled up to 5$\times$; only 2 bins fall below the 1,000 minimum threshold (LATIN:sw|LATIN:sw with 948 samples, ARABIC:ur|ARABIC:ur with 682) and are dropped. This yields \textbf{27.6 million balanced pairs} for triplet generation.

From these pairs, I generate triplets for contrastive training:
\begin{itemize}
    \item \textbf{Phase 1 triplets:} Triplets with script-aware random negatives (80\% same-script, 20\% global) sampled from the full toponym pool
    \item \textbf{Phase 3 triplets:} Triplets with hard negatives---phonetically similar names (high PanPhon cosine similarity, same script) from different places, with no shared attestations
\end{itemize}
...
\paragraph{Effective Training Set.} From the 27.6 million balanced pairs, I generate triplets using script-aware random negatives. Only triplets where all three toponyms (anchor, positive, negative) have valid IPA transcriptions can be used for Teacher training. From 15.9 million unique toponym IDs referenced in pairs, the filtering yields \textbf{27,559,050 Phase 1 triplets}, split deterministically into \textbf{24,803,658 training} and \textbf{2,755,392 validation} samples (90:10 ratio).

The substantial increase from v3's 467K usable triplets to v4's 27.6M reflects two improvements: (1) the HDBSCAN clustering generates more phonetically coherent pairs from the same corpus; (2) using 80\% same-script negatives increases the probability that anchor, positive, and negative all have IPA coverage since same-script languages often share Epitran support.
