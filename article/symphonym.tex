...
The creation of these 100 custom Epitran extension files was assisted by large language models (Claude, Gemini, and ChatGPT), which generated and cross-checked IPA mappings based on linguistic descriptions and Unicode character tables. None of these outputs has been validated by human experts; they serve as provisional mappings to enable v6 training. Future work may involve expert review and refinement of these rules.
...
\paragraph{Note on Chinese Topolects and Hebrew.} Three Chinese varieties (Wu, Gan, Yue) with 49k, 37k, and 33k toponyms respectively cannot be reliably mapped to IPA using rule-based approaches due to insufficient linguistic resources. These will be handled via CharsiuG2P \citep{zhu2022charsiu-g2p}, a neural G2P model. Similarly, Hebrew toponyms present a unique challenge: unpointed Hebrew script (the standard in historical documents) omits vowels, making rule-based phonetic transcription unreliable. For v6, Hebrew text will be processed using Phonikud \citep{kolani2025phonikudhebrewgraphemetophonemeconversion}\footnote{\url{https://github.com/thewh1teagle/phonikud}}, which directly generates International Phonetic Alphabet (IPA) transcriptions from unpointed Hebrew text using a neural model. The resulting IPA strings are then passed to PanPhon for generation of 192-dimensional articulatory feature vectors. This two-stage pipeline (Phonikud $\rightarrow$ PanPhon) enables accurate phonetic embedding generation for the 145k Hebrew toponyms without requiring intermediate Epitran conversion.
