...
\begin{enumerate}
    ...
    \item Apply HDBSCAN clustering with \texttt{min\_cluster\_size=2} and \texttt{min\_samples=2} to identify natural phonetic groupings
...
Real-world examples demonstrate the power of this approach. A London place record (wd:Q1001456) with 21 multilingual toponyms yields 5 clusters that respect phonetic boundaries across scripts:

\begin{itemize}
    \item \textbf{Arabic cluster (3 members):} \textar{لندن} (fa), \textar{لندن} (ar), \textar{لندن} (ur)---identical orthography across Persian, Arabic, and Urdu, with intra-cluster similarity of 0.98
    \item \textbf{Cyrillic-Latin cross-script cluster:} Лондон (ru/uk) clusters with Latin transcriptions London (hu, cs, es, it, tr) at similarity 1.00, demonstrating successful phonetic alignment
    \item \textbf{Noise detection:} Bengali \textbn{লন্ডন}, Chinese \textcn{伦敦}, and certain Latin variants (London@en, London@pt) are correctly classified as noise---phonetically distinct singletons that should not form training pairs
\end{itemize}

A Moscow place record (wd:Q499927) with 33 multilingual toponyms across 11 scripts yields 9 clusters that respect both script and phonetic boundaries:

\begin{itemize}
    \item \textbf{Indic cluster (3 members):} Devanagari \texthi{मॉस्को} (mr/hi) and Bengali \textbn{মস্কো} cluster together with similarity 0.97, reflecting shared phonetic structure across related scripts
    \item \textbf{South Indian cluster:} Tamil \textta{மாஸ்கோ} and Telugu \textte{మాస్కో} form their own cluster (similarity 0.98)
    \item \textbf{Cross-script alignment:} Cyrillic Москоу (ru) clusters with Latin forms Moscow (ro, pt, es) at similarity 0.97
    \item \textbf{Noise:} Georgian \textge{მოსკოვი}, Thai \textth{มอสโก}, and Arabic variants are classified as noise---phonetically isolated from other forms
\end{itemize}

A Beijing place record (gn:1816670) with 36 toponyms demonstrates cross-script phonetic alignment: Chinese \textcn{北京} (zh) clusters directly with Korean \textko{베이징} (ko) at similarity 1.00, while ``Peking'' variants (Peking@nl, Peking@cs, Peking@de) form a separate cluster from ``Beijing'' variants (Beijing@ro, Beijing@de). Georgian \textge{პეკინი} clusters with Italian ``Pechino'' (similarity 0.90), capturing the shared /pekin/ phonetic core despite different scripts.
