...
\subsection*{Completed: Epitran Extension Program}
...
\paragraph{Root Cause.} Epitran\footnote{\url{https://github.com/dmort27/epitran}} (the rule-based grapheme-to-phoneme converter used to generate IPA transcriptions for PanPhon feature extraction) lacked phonetic mapping tables for many language-script pairs represented in WHG. While Epitran officially supports 100+ languages, the specific combinations needed for historical toponymy (e.g., \texttt{ell-Grek} for Modern Greek, \texttt{hye-Armn} for Armenian) were not included.

\paragraph{Solution Implemented.} Between mid-January and late January 2026, I extended Epitran by creating 100 new language-script mapping CSV files stored in \texttt{phonetics/epitran\_extensions/}. These mappings cover:

\begin{itemize}
    \item \textbf{Critical missing scripts:} Armenian (\texttt{hye-Armn}), Greek (\texttt{ell-Grek}), Gujarati (\texttt{guj-Gujr}), Georgian (\texttt{kat-Geor}), and numerous Indic and non-Latin scripts.

    \item \textbf{High-frequency under-resourced language pairs:} Including Norwegian Bokmål (\texttt{nob-Latn}), Scottish Gaelic (\texttt{gla-Latn}), Basque (\texttt{eus-Latn}), Egyptian Arabic (\texttt{arz-Arab}), and 80+ others with toponym counts ranging from 10,000 to 450,000.

    \item \textbf{Specialized historical and minority languages:} Such as Old Prussian (\texttt{prg-Latn}), Cajun French (\texttt{frc-Latn}), Romansh (\texttt{roh-Latn}), and various creoles.
\end{itemize}
...
\begin{itemize}
    ...
    \item \texttt{CHARSIU}: Chinese topolects (Gan, Wu, Yue) to be handled by CharsiuG2P fallback
    \item \texttt{PHONIKUD}: Hebrew (145k toponyms)---unpointed Hebrew text lacks vowels, making deterministic IPA transcription impossible without a vocalization system like Nikud or a neural fallback
...
\paragraph{Note on Chinese Topolects and Hebrew.} Three Chinese varieties (Wu, Gan, Yue) with 49k, 37k, and 33k toponyms respectively cannot be reliably mapped to IPA using rule-based approaches due to insufficient linguistic resources. These will be handled via CharsiuG2P \citep{zhu2022charsiu-g2p}, a neural G2P model. Similarly, Hebrew toponyms present a unique challenge: unpointed Hebrew script (the standard in historical documents) omits vowels, making rule-based phonetic transcription unreliable. For v6, Hebrew text will be processed using Phonikud \citep{kolani2025phonikudhebrewgraphemetophonemeconversion}\footnote{\url{https://github.com/thewh1teagle/phonikud}}, which generates International Phonetic Alphabet (IPA) transcriptions directly from unpointed Hebrew text.
...
\paragraph{Enhanced Effective Training Set.} From the 65.1 million positive pairs generated via HDBSCAN clustering across 8.2 million places (forming 18.3 million phonetic clusters, with 4.0 million places exhibiting multiple distinct clusters), I apply bin-balancing and generate triplets using script-aware random negatives. Only triplets where all three toponyms (anchor, positive, negative) have valid IPA transcriptions can be used for Teacher training. From 15.9 million unique toponym IDs referenced in pairs, the filtering yields \textbf{27,559,050 Phase 1 triplets}, split deterministically into \textbf{24,803,658 training} and \textbf{2,755,392 validation} samples (90:10 ratio).
