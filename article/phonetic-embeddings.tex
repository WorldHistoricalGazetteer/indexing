...
% Illustrative distribution based on reported percentiles
    ...
    The phonetic similarity model is to be deployed as a core component of
    ...
    reconciliation infrastructure. Embeddings have now been computed for all 68.7
...
%% ============================================================================
    ...
    The divergent performance on MEHDIE testsets reveals a fundamental tension in
    cross-lingual matching: models optimised for global consistency necessarily
    sacrifice sensitivity to language-family-specific patterns.
    ...
    function. This explains the performance gap on TS7 and TS10, where Semitic
    morphophonology dominates.

    However, specialisation has costs. The MEHDIE transliteration library required
    substantial linguistic expertise to develop and remains specific to one
    language pair. This approach scales to any language with sufficient gazetteer
    coverage, requiring no manual rule engineering. The strong performance on TS8
    (+0.058 F5 over MEHDIE) suggests that when difficulty stems from source noise
    rather than systematic morphology, learned representations outperform
    hand-crafted rules.
