...
\section*{Remaining Work After ES Comes Online}
\begin{enumerate}
\item Review extraction\_stats.json for actual numbers
\item Update Section 4.4 of the .tex with new statistics
\item Proceed with model training
\end{enumerate}
...
\paragraph{Training Pair Generation.} Positive pairs are generated from co-located toponyms within each place record. A place entry for ``Paris'' in Wikidata, for instance, contains variants in dozens of languages: Paris (en/fr), París (es), Париж (ru), \textar{باريس} (ar), \textcn{巴黎} (zh), \textcn{パリ} (ja), etc. We generate pairs from all combinations of co-located toponyms that pass phonetic similarity filtering (normalised Levenshtein $\geq 0.35$ after romanisation). With approximately 15 million places containing two or more toponyms, and an average of 8 valid pairs per such place after filtering, we estimate 100--120 million training pairs, of which approximately 45\% are cross-script. This yields abundant training signal---two orders of magnitude more than comparable toponym matching systems.
