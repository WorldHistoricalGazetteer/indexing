...
Positive pairs are generated dynamically during training from co-located name attestations sharing geographic coordinates. After deduplication, 2.55 million unique pairs remain, of which 45.3\% are cross-script. Analysis of pair frequency revealed significant redundancy from common place names (Georgetown, Springfield, Salem) appearing across multiple locations. Additionally, same-string pairs (e.g., ``Georgetown'' $\leftrightarrow$ ``Georgetown'' for different places) provide no phonetic learning signal. \placeholder Future iterations will filter such pairs to focus training on phonetically informative examples.
...
\placeholder Phase 3 noise: asymmetric local perturbation applied to anchors only, intended to encourage invariance within an already-defined equivalence class, not to introduce new phonetic evidence.
...
The Student alone is fine-tuned using triplet loss, with the Teacher's role complete after Phase 2 distillation.
