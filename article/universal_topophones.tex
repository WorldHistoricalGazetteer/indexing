...
\bigskip
We deliberately exclude OpenStreetMap (OSM) data from training, despite its availability in WHG's places index, because it is  less-rigorously curated than the scholarly sources. The WHG places index also includes data from Wikidata, the Getty Thesaurus of Geographic Names (TGN), and contributor-curated collections, but these either substantially overlap with GeoNames or serve different purposes in the reconciliation pipeline.
