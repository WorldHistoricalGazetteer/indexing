...
\toprule
\textbf{Feature} & \textbf{Rama (2016)}  & \textbf{Mueller \& Thyagarajan (2016)} & \textbf{Our Approach} \\
\midrule
Input Representation & Character embeddings & Word/sentence embeddings & Articulatory feature vectors (PanPhon) \\
\addlinespace
Phonetic Knowledge & Implicit (learned from data) & None (semantic focus) & Explicit (engineered features) \\
\addlinespace
Distance Metric & Contrastive loss (Euclidean) & Manhattan ($L_1$) & Cosine + triplet margin \\
\addlinespace
Primary Strength & Minimal preprocessing & Gradient stability & Cross-lingual generalisation \\
\addlinespace
Script Handling & Single script & Single script & 20 scripts unified \\
\bottomrule
\end{tabular}
\caption{Comparison of neural approaches to string similarity. Our use of explicit articulatory features and cosine-based similarity enables cross-script phonetic matching without requiring implicit learning of phonetic relationships from large corpora.}
