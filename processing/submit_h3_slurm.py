...
def _build_sbatch_script(
    *,
    ...
    lines = [
        ...
        "    --namespace \"$NAMESPACE\"",
        "",
        "# Compact per-namespace H3 coverage to staged/_aggregates/, consumed by",
        "# Batch 11 (inventory push) via push_gazetteer_inventory._read_h3_coverage.",
        "# NOT read by ccode enrichment: build_un_prefilter builds cell->ccode from",
        "# the un docs' own geometries[].h3_cover and never opens this file. An",
        "# earlier version of this comment claimed a Batch 7 consumer, which sent",
        "# a later audit hunting for a second code path that does not exist.",
