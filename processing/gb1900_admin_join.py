...
lacking a facet (not in the gazetteer, or an empty parish), a **Voronoi-containment fallback**
(SG 2026-07-18): the labelled points implicitly tessellate space into Voronoi cells, so an
untagged point takes the facet of the region it falls in = its nearest labelled point. We use
...
def build_kdtree(gaz, facet):
    """cKDTree of the gazetteer points that HAVE `facet`, + aligned value array (Voronoi sites)."""
    from scipy.spatial import cKDTree
    pts, vals = [], []
    for g in gaz.values():
        if g["lon"] is not None and g[facet]:
            pts.append((g["lon"], g["lat"])); vals.append(g[facet])
    pts = np.asarray(pts, dtype=float)
    print(f"[admin] {facet}: KDTree over {len(pts):,} labelled points", flush=True)
    return cKDTree(pts), np.asarray(vals, dtype=object)
...
def run(a):
    ...
    # pass 2 — Voronoi-containment fallback (k-NN majority) per facet
    from collections import Counter
    for f in FACETS:
        if not need[f]:
            continue
        tree, vals = build_kdtree(gaz, f)
        q = np.array([(recs[i][2], recs[i][3]) for i in need[f]], dtype=float)
        _, nn = tree.query(q, k=min(a.knn, len(vals)))     # k nearest labelled points
        nn = np.atleast_2d(nn)
        agree = maj = 0
        for row, i in enumerate(need[f]):
            cnt = Counter(vals[j] for j in nn[row])
            val, c = cnt.most_common(1)[0]
            if c == nn.shape[1]:                            # all k agree
                recs[i][4][f] = val; recs[i][5][f] = "interp"; agree += 1
            elif c > nn.shape[1] / 2:                       # majority
                recs[i][4][f] = val; recs[i][5][f] = "interp_maj"; maj += 1
        print(f"[admin] {f}: filled {agree:,} (k agree) + {maj:,} (majority) "
              f"of {len(need[f]):,}", flush=True)

    # write patch
    stats = {f: {"gazetteer": 0, "interp": 0, "interp_maj": 0, "none": 0} for f in FACETS}
...
def main(argv=None):
    ...
    p.add_argument("--knn", type=int, default=7, help="k for Voronoi-containment k-NN majority fill")
