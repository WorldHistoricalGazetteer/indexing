...
def generate_geoscheme(dry_run=False):
    ...
    try:
        ...
        for name, info in M49_SUBREGIONS.items():
            ...
            # them across the entire globe at low zoom. Split at +/-180 here so
            # each region is a multipolygon with parts on each side of the
            # dateline.
            union = split_at_antimeridian(make_valid(unary_union(geoms)))
