...
| 10 | Tile generation (no ES) — runs **ahead of** Global Barrier | Done — tilesets generated (and OSM/OHM admin tiles re-run on 2026-05-05 after the geom_store wipe; see 2026-05-25 snapshot). **2026-05-27: the tileserver `config.json` rewrite is now codified** as `processing/update_tileserver_config.py` (push → single safe merge-rewrite preserving all other entries → restart → serving-verify → rollback) and wired into `submit_tiles_slurm`'s `afterok` finalize — previously this was an unrecorded manual edit, so gazetteer mbtiles were pushed but never served. |
| 11 | Index loaders + **gazetteer inventory push (final-step gating)** | Done — `index_from_stage` loaded all 16 namespaces into `places_postbarrier-…`, alias swapped. Inventory push **verified** (2026-05-25): registry pushed to **both** stacks on 2026-05-04 — `GazetteerRegistryEntry` holds 24 gazetteers (matching counts) on prod-main *and* dev-atlas. (The push writes no marker, hence the earlier "unverified"; confirmed by reading both Postgres DBs.) **2026-05-27:** `push_gazetteer_inventory` gained a `--namespace` single-namespace mode (incremental push, no full-run inventory) and a **tileserver serving preflight gate** (refuses to push unless each authority's tileset serves) — the enforceable form of "serving confirmed before the manifest" given the push is a manual step. The `GazetteerInventoryView` upsert is cumulative (per-entry `update_or_create`; single-namespace pushes don't wipe others). |
