...
| **S9** (cont.) | 2.9 | Code-only residuals that touch nothing 2.7 executes: the `_unlink_quietly` narrowness test, `og`'s missing `configure_module_writer` (4.2), `backfill_uncoded_ccodes` excluding `ukhc` (audit §3b), and a specification for the directory-symlink swap. ⚠️ **Resolver hoisting WITHHELD** until 2.7 lands — it edits five files including `h3_merge`/`ccode_merge`/`index_from_stage`. | ◐ **running**, parallel to 2.7 |
