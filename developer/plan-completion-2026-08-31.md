...
| **S8** | 2.7 | Give `gn`/`wd`/`nl` a real `final/` — `h3_stage` → `h3_merge` → ccode → `ccode_merge`. The correct route, not the fast one: `wd`/`nl` get genuine ccode enrichment. **Gates S5.** | ◐ **holding** — blocked behind the overlay harvest's concurrent read of staged `gn`/`wd`; no work started |
