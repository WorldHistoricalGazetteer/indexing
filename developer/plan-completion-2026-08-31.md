...
🛑 **`source` HAS A WIDER TRAP THAN ONE EXCEPTION: a grep for `"source":`
returns ELEVEN hits of which exactly ONE should change** (verified 3 Sep):

```
ROOT — DELETE (1):
  tgn-places.py:298      "source": "tgn"      beside "namespace": "tgn"
GEOMETRY-LEVEL — KEEP (4):
  alcedo-places.py:389   ge = {... "source": "hgis" ...}
  alcedo-places.py:398   geom_entry["source"] = NAMESPACE
  ottgaz-places.py:307   ge["source"] = "ofs"
  hgis-places.py:169     ge["source"] = NAMESPACE
NOT A FIELD AT ALL (6):
  six files in processing/ — "script": {"source": ...} is PAINLESS syntax
```

✅ **`geometries.source` was already declared in the schema**, which
independently confirms the geometry-level field is legitimate and only the root
one accidental. **Fix `tgn-places.py:298` BY LINE, NOT BY PATTERN.**
...
✅ **PACKAGE 2 COMPLETE 3 Sep** — `1e982d1` · `c66caa3` · `c9970aa`.
**All schema diffs purely ADDITIVE; zero deletions across all three commits.**
Group D came to **15 fields declared, not 12 removed** — measured from the live
root mapping (42 fields) against the schema rather than taken from the triage
list, which had undercounted. ⚠️ **Correction: `description` is `nl` ONLY,
2,057 docs** — the census said "nl + whg" and `whg` contributes none; all 2,057
are `native-land.ca` URLs, `descriptions` null on every one, `links` null.

✅ **ROOT `timespans` — RESOLVED BY DECLARING IT; the planned migration was
unexecutable.** SG's option (a), `c66caa3`. The plan said *migrate the 202 into
a nested path, then drop the field*. On inspection that could not be done and
should not have been:

* **all 202 carry ZERO geometries**, so `geometries[].timespans` cannot receive
  them — the specified target does not exist;
...
* the field is **not a stray**: confined to `chgis` (81,292) and `dgsd` (1,216),
  and it is `dgsd`'s normal convention. The 202 are not malformed — they are the
  subset with nothing else dated.

Declared `nested`, identical to its three siblings. Nothing migrated, no writer
changed, **the 202-at-risk problem dissolves**. Same shape as `depictions`.

⚠️ **CARRY TO THE NEXT FULL REBUILD:** the live index maps root `timespans`
**dynamically as `object`, not `nested`**, with only `start.in`/`end.in` — so
`earliest`/`latest` are absent and **nested queries against the root path fail
on the current index.** The schema now matches its siblings; the mapping
converges only at the next full rebuild. **Re-check it there.**
...
(`c66caa3`) and each newly declared field is documented against the production
code that reads it. **Four of its six sections have always been `_TBD_`**, and
they want *different* dispositions rather than one sweep:

| section | disposition |
|---|---|
| `toponyms.json` | ⭐ **Write it — the valuable one.** `attestations`, `phon_emb`, `namespaces` are non-obvious and load-bearing: §4.18's whole verdict turned on discovery reading `attestations` and **not** the nested array. That knowledge should not live only in an audit transcript. |
| `clusters.json` | 🛑 **CLOSE as won't-document, do not fill.** Its own note says document only if it outlives the re-architecture — it did not. `CLUSTERS_INDEX` is gone from `gateway/config.py` and the index is legacy. Writing it up documents a corpse. |
| `types.json` | ⚠️ **Check the type-system skill first.** It may already cover the AAT hierarchy, materialised paths and cross-vocabulary mappings — writing a second account creates a source that drifts. |
| pipelines | A line each is probably enough. |

**Also fold in:**
* the stale open question at the top — `geometries[].bounds` array order
  (`[minLon, minLat, maxLon, maxLat]`?), trivially checkable and long unanswered;
* ⚠️ **the root-`timespans` live-mapping caveat as a REBUILD CHECKPOINT** — it
  expires at the next full rebuild and must be re-checked *there*, not assumed
  to have converged.
