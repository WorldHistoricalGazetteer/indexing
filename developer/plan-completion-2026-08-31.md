...
returns ELEVEN hits of which exactly ONE should change** (verified S5, 3 Sep):
...
**Fix `tgn-places.py:298` BY LINE, NOT BY PATTERN.** The four geometry-level
writes are legitimate per-geometry provenance and must stay; the six Painless
hits are ES scripting and are not fields at all. An earlier note here named only
`alcedo:389` and was understated.
...
* **S4:** 2.5 and 2.6 are both finishable cold — *"the residual risk is not
  ...
  gap or `index_namespace`'s stage chain (4.11).⚠️ **THE 202 RESTS ON A PREDICATE WORTH STATING, because a stricter one gives
36.** Two defensible measurements of "would lose its only temporal data":

```
root timespans (any)                                     82,508
  A) no geometries + toponyms + RELATIONS timespans          36   (all dgsd)
  B) no geometries + toponyms timespans                     202   (all dgsd)
```

The difference is **`relations.timespans`**. `dgsd:10020` is in B and not in A,
because both its relation entries carry dates. **B — 202 — is the right
predicate**: a relation's timespan describes the **edge** ("part of X,
978–1086"), not the place's own lifespan, so such a place really does lose its
own temporal extent to a root-field deletion even though dates survive on its
relations. Migrating all 202 is the conservative choice and the 166 with
relation dates lose nothing by inclusion. **All 202 are `dgsd`**, which makes
the writer fix single-namespace.

⚠️ **`source` HAS A WIDER TRAPAugust re-ingestion
