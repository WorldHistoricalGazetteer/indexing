...
import os, json, base64, io, glob, numpy as np
...
gly, lett, capp, styy = d["glyphs"], d["letter"], d["cap"], d["style"]   # bind ONCE (indexing d[...] re-decompresses + leaks views)
alpha = [dict(L=str(lett[i]), cap=bool(capp[i]), style=str(styy[i]), glyph=gly[i].copy()) for i in range(len(gly))]
...
perfont = defaultdict(list); pairbox = defaultdict(list); nproc = 0; MAXPROC = 12000
...
for line in lines:
    ...
    if nproc % 2000 == 0:
        ...
        print(f"  processed {nproc}; fonts full {full}/{len(ALLF)}", flush=True)
