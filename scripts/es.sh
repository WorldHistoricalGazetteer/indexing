...
case "$1" in
    ...
    *)
        ...
        echo "      The GPU job SUBMITS the SMP index job itself on completion"
        echo "      (cross-cluster Slurm dependencies do not work on CRC), so you"
        echo "      do NOT need -update-embeddings-index afterwards."
        echo "      Env: TOPONYMS_DB, TOPONYMS_TARGET_INDEX (default index name:"
        echo "           'toponyms'; a rebuild wants toponyms_<run_id>),"
        echo "           GPU_QOS/GPU_CONSTRAINT/GPU_REQUEUE (auto-set for"
        echo "           --partition preempt, usually the fastest route to a GPU)"
        echo
        echo "  -update-embeddings-index VERSION"
        echo "      Index embeddings to ES (CPU). Normally submitted automatically"
        echo "      by -update-embeddings; run it directly only when compute was"
        echo "      done separately, or to re-index from an existing parquet."
